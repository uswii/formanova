...
export function CinematicHero({ images, className }: CinematicHeroProps) {
  ...
  const rafWheel = useRef<number>(0);

  // Scroll-based image transition — throttled via rAF
  useEffect(() => {
    let accumulatedDelta = 0;
    const scrollThreshold = 150;
    
    const handleWheel = (e: WheelEvent) => {
      if (rafWheel.current) return; // skip if a frame is already queued
    ...
    return () => {
      ...
      if (rafWheel.current) cancelAnimationFrame(rafWheel.current);
    };
  }, [images.length, isTransitioning]);

  // Track scroll for parallax effects — throttled via rAF
  useEffect(() => {
    const handleScroll = () => {
